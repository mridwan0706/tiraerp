...
{
    public partial class frmSOManualEntryEditor : Form
    {
        ...
        private void frmSOManualEntryEditor_Load(object sender, EventArgs e)
        {
            ...
            {
                dtTglDelv.MinDate = g_dTglGudang;
                dtTglExp.MinDate = g_dTglGudang;
            }
            ...
            if (clsGlobal.MODE_TRX == 1)
            {
                initDateDelvExp(Convert.ToDateTime(dtTglOrder.Value).ToString("yyyyMMdd"), "", "", 1);
        ...
        private void btnPopUpOutlet_Click(object sender, EventArgs e)
        {
            ...
            initDateDelvExp(Convert.ToDateTime(dtTglOrder.Value).ToString("yyyyMMdd"), txtKdOutlet1.Text, lblKdOutlet2.Text, 1);
        }
                        ...
                        else
                        {
                            if (dtFill1.Rows[0]["gh_group_menu_id"].ToString().Trim() != "C")
                            {
                                ...
                                for (lCounter = 0; lCounter < cbTOP.Items.Count; lCounter++)
                                {
                                    ...
                                    int kiri = tops.IndexOf("~") + 2; //mengambil angka di tengah TOP
                                    int kanan = tops.IndexOf("-", kiri) - 1;
                                    string stops = tops.Substring(kiri, kanan - kiri);
                                    if (stops == "0")
                                    {
                                        bExists = true;
                                        cbTOP.SelectedIndex = 0;
        ...
        private void dgvSalesDetail_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (dgvSalesDetail.Rows[row].Cells["wsd_prd_master_code"].Selected == true)
                {
                    ...
                    //if (isNefoKAM)
                    //{
                    //    strSQL = strSQL + " LEFT JOIN SO_TAX_CODE WITH(NOLOCK) ON  CAST(CONVERT(DATE, '" + dtTglOrder.Value.ToString("yyyyMMdd") + "', 112) AS DATETIME) >=cast(stc_tax_valid_from as datetime) ";
                    //    strSQL = strSQL + "  AND CAST(CONVERT(DATE,'" + dtTglOrder.Value.ToString("yyyyMMdd") + "', 112) AS DATETIME) <=cast(stc_tax_valid_to as datetime)  ";
                    //}
                    //else
                    //{
                    //    strSQL = strSQL + " LEFT JOIN SO_TAX_CODE WITH(NOLOCK) ON prm_tax_code = stc_tax_code ";
                    ...
                    if (dtFillC.Rows.Count > 0)
                    {
                        if (pCheckValue(dtFillC.Rows[0].ToString(), dgvSalesDetail.Rows.Count, dgvSalesDetail) == true)
                        {
                            if (isNefoKAM)
                            {
                                ...
                                {
                                    MessageBox.Show("Parameter untuk so_tax_code valid date '" + dtTglOrder.Value.ToString("yyyyMMdd") + "' tidak ditemukan!.", clsGlobal.APP_MSG_CAPTION, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                                    return;
                                }
        ...
        private void dgvSalesDetail_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 13)
            {
                dtTglPrice = new DateTimePicker();
                //dtTglPrice.ShowCheckBox = true;
                dtTglPrice.Checked = true;
                ...
                if (isNefoKAM)
                {
                    ...
                    dtTglPrice.MinDate = g_CreateAtSAP;

                }

                if (clsGlobal.MODE_TRX == 2)
                {
                    dtTglPrice.Value = Convert.ToDateTime(dgvSalesDetail.Rows[e.RowIndex].Cells["tgl_price"].Value);
                }
                ...
                dtTglPrice.Format = DateTimePickerFormat.Custom;
                dtTglPrice.CustomFormat = "dd MMM yyyy";
                ...
                dtTglPrice.CloseUp += new EventHandler(dtTglPrice_CloseUp);
            ...
            if (e.ColumnIndex == 25)
            {
                ...
                //Adding DateTimePicker control into DataGridView   
                dgvSalesDetail.Controls.Add(cmbReasonGrid);

                // Setting the format (i.e. 2014-10-10)  
                //oDateTimePicker.Format = DateTimePickerFormat.Custom;
                // oDateTimePicker.Value = DateTime.Now;
                //oDateTimePicker.CustomFormat = "dd MMM yyyy";

                // It returns the retangular area that represents the Display area for a cell  
                Rectangle oRectangle = dgvSalesDetail.GetCellDisplayRectangle(e.ColumnIndex, e.RowIndex, true);

                //Setting area for DateTimePicker Control  
                cmbReasonGrid.Size = new Size(oRectangle.Width, oRectangle.Height);

                // Setting Location  
                cmbReasonGrid.Location = new Point(oRectangle.X, oRectangle.Y);

                // An event attached to dateTimePicker Control which is fired when DateTimeControl is closed  
                cmbReasonGrid.DropDownClosed += new EventHandler(cmbReasonGrid_CloseUp);
                cmbReasonGrid.Leave += new EventHandler(cmbReasonGrid_CloseUp);

                // An event attached to dateTimePicker Control which is fired when any date is selected  
                cmbReasonGrid.TextChanged += new EventHandler(cmbReasonGrid_OnTextChange);


                // Now make it visible  
                cmbReasonGrid.Visible = true;
                //cmbReasonGrid.Focus();         
                double QtyReal = 0;
                double QtyReq = 0;
                if (e.RowIndex > -1 && txtKdOutlet1.Text != "" && dgvSalesDetail.Rows[e.RowIndex].Cells["wsd_prd_master_code"].Value.ToString() != "" && dgvSalesDetail.Rows[e.RowIndex].Cells["wsd_so_xqty"].Value.ToString() != "")
                {
                    ...
                    {
                        cmbReasonGrid.DataSource = null;
                        dgvSalesDetail.Rows[e.RowIndex].Cells["reason"].ReadOnly = true;
                        dgvSalesDetail.Rows[e.RowIndex].Cells["reason"].Value = "";
        ...
        private void cmbReasonGrid_OnTextChange(object sender, EventArgs e)
        {
            ...
            if (cmbReasonGrid.SelectedValue.ToString() != "")
            {
                dgvSalesDetail["reason", i].Value = cmbReasonGrid.SelectedValue.ToString();
            }
            else
            {
        ...
        {
            int i = dgvSalesDetail.CurrentRow.Index;
            if (dtTglPrice.Checked)
            {
                dgvSalesDetail.Rows[i].Cells["tgl_price"].Value = dtTglPrice.Value.ToString("dd MMM yyyy");
            }
            else
            {
        ...
        private void dgvSalesDetail_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (e.ColumnIndex == 0)
            {
                ...
                if (dgvSalesDetail.Rows[row].Cells["wsd_prd_master_code"].Value.ToString() != "")
                {
                    ...
                    DataTable dtFill3 = new DataTable();
                    bool isFlagFifaActive = IsActiveFlagSLED();
                    ...
                    if (isFlagFifaActive)
                    {
                        strSQL = strSQL + ", mcps_prd_sled as [sled]";
                    }
                    strSQL = strSQL + " from IM_PRD_MASTER WITH (NOLOCK)  ";
                    if (isNefoKAM)
                    {
                    ...
                    //strSQL = strSQL + " WHERE prm_status = 'A'";
                    strSQL = strSQL + " WHERE prm_status IN ('A','D')";
                    strSQL = strSQL + " AND ( prm_prd_cust_flag = 'N' OR ( prm_prd_cust_flag = 'Y' AND impc_cust_code1 IS NOT NULL ) ) and prm_prd_master_code='" + dgvSalesDetail.Rows[e.RowIndex].Cells[e.ColumnIndex].Value.ToString() + "'";
                    //if (isMultisource)
                    //{
                    //    if (cbFlagSloc.SelectedValue.ToString().ToUpper().Equals("COC"))
                    //    {
                    ...
                    if (dtFill3.Rows.Count > 0)
                    {
                        ...
                        if (pCheckValue(dgvSalesDetail.Rows[row].Cells["wsd_prd_master_code"].Value.ToString(), row, dgvSalesDetail) == true)
                        {
                            ...
                            if (chkSLED.Checked)
                            {
                                dgvSalesDetail.Rows[row].Cells["wsd_sled"].ReadOnly = false;

                                if (IsActiveFlagSLED())
                                {
                                    dgvSalesDetail.Rows[row].Cells["wsd_sled"].Value =
                                        dtFill3.Rows[0]["sled"].ToString().Trim();
                                }
                                else
                                {
                                    dgvSalesDetail.Rows[row].Cells["wsd_sled"].Value = "";
                            ...
                            {
                                dgvSalesDetail.Rows[row].Cells["wsd_sled"].Value = "";
                                dgvSalesDetail.Rows[row].Cells["wsd_sled"].ReadOnly = true;
                            }
                            pChangePCode(row);
            ...
            if (e.ColumnIndex == 28)
            {
                if (dgvSalesDetail.Rows[row].Cells["wsd_prd_master_code"].Value.ToString() != "")
                {
                    ...
                    if (dgvSalesDetail.Rows[row].Cells["wsd_req_order_xqty"].Value.ToString() != "")
                    {
                        ...
                        {
                            dgvSalesDetail.Rows[e.RowIndex].Cells["reason"].ReadOnly = false;
                        }
        ...
        private void pChangePCode(int iIndex)
        {
            ...
            string _dttglprice = dtTglOrder.Value.ToString("dd MMM yyyy");
            ...
            dgvSalesDetail.Rows[iIndex].Cells["wsd_sled"].Value = "";
        ...
        private bool fSaveSO(string sCredLimit, bool bCalcDisc)
        {
            ...
            try
            {
                ...
                pClearDiscount();
                pEmptyTPRFlag();
                DateTime _dtgl1 = DateTime.Parse(dtTglOrder.Value.ToString(), CultureInfo.GetCultureInfo("en-US"));
                DateTime _dtglDelv = DateTime.Parse(dtTglDelv.Value.ToString(), CultureInfo.GetCultureInfo("en-US"));
                DateTime _dtglExp = DateTime.Parse(dtTglExp.Value.ToString(), CultureInfo.GetCultureInfo("en-US"));
                ...
                strSQL = strSQL + dtTglValidasi.Value.ToString("yyyy-MM-dd") + "' , '"; //'wsh_validate
                ...
                strSQL = strSQL + dtTglPO.Value.ToString("yyyy-MM-dd") + "','"; //'wsh_po_date
        ...
        private bool fSaveProsesSO(string sCredLimit, bool bYesNo)
        {
            ...
            try
            {
                ...
                DateTime _dtgl2 = DateTime.Parse(dtTglOrder.Value.ToString(), CultureInfo.GetCultureInfo("en-US"));
                //'Get Year, Periode, Week untuk Tgl SO
                DataTable dtFill2 = new DataTable();
                strSQL = "select fwd_year, fwd_periode, fwd_week_no  from TBL_GS_WORKING_DAY WITH (NOLOCK)  ";
                ...
                {
                    MessageBox.Show("Tidak ada tanggal " + dtTglOrder.Value.ToString("dd MMM yyyy") + " pada tanggal hari kerja ", clsGlobal.APP_MSG_CAPTION, MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    keyFound = false;
                    return keyFound;
                }
                ...
                strSQL = strSQL + idO + "' , "; //'wsh_pay_type
                strSQL = strSQL + "0 , '"; //'wsh_gross_net_amt
                strSQL = strSQL + Convert.ToDateTime(dtTglValidasi.Value).ToString("yyyy-MM-dd") + "' , '"; //'wsh_validate
                strSQL = strSQL + txtNoPO.Text + "' , '"; //'wsh_po_no
                if (xFlagDC == "Y")
                {
                    //if (strFlagLimit != "")
                ...
                string idOO = Convert.ToString(selectedDataRow1["Code"]);
                strSQL = strSQL + idOO + "' , '"; //'wsh_top_id
                strSQL = strSQL + Convert.ToDateTime(dtTglPO.Value).ToString("yyyy-MM-dd") + "','"; //'wsh_po_date
                strSQL = strSQL + lLeadTime + "'"; //'wsh_cust_leadtime
                if (pdaFlag == "" || pdaNO == "")
                {
                    //strSQL = strSQL + "' , '"; 
        ...
        {
            if (dgvSalesDetail.CurrentCell.ColumnIndex == 9)
            {
            ...
            if (dgvSalesDetail.CurrentCell.ColumnIndex == 28)
            {
        ...
        private void chkSLED_CheckedChanged(object sender, EventArgs e)
        {
            ...
            if (chkSLED.Checked)
            {
                ...
                // Jika FLAGFIFA = Y, isi default SLED dari mapping
                if (IsActiveFlagSLED())
                {
                    ApplyMappedSLEDToAllRows();
                }
            }
            else
            {
                // Jika unchecked, semua SLED harus blank dan tidak bisa diisi
                ClearAllSLED();
            }
        }
        ...
        private bool IsActiveFlagSLED()
        {
            DataTable dtGS = _clsGlobal.ExecDT(@"
                              SELECT TOP 1 gh_function_code
                              FROM GS_GEN_HARDCODED WITH (NOLOCK)
                              WHERE gh_function_name = 'FLAGFIFA'
                                AND gh_sys = 'H'
                          ");

            return dtGS.Rows.Count > 0
                && dtGS.Rows[0]["gh_function_code"].ToString().Trim().ToUpper() == "Y";
        }

        private void ApplySLEDFlagToRow(int rowIndex)
        {
            bool isFlagFifaActive = IsActiveFlagSLED();

            if (!isFlagFifaActive)
            {
                dgvSalesDetail.Rows[rowIndex].Cells["wsd_sled"].Value = "";
                dgvSalesDetail.Rows[rowIndex].Cells["wsd_sled"].ReadOnly = true;
            ...
            dgvSalesDetail.Rows[rowIndex].Cells["wsd_sled"].ReadOnly = !chkSLED.Checked;
        ...
        private string GetMappedSLED(string pcode, string grade, string size)
        {
            if (!IsActiveFlagSLED())
                return "";

            string sql = "";
            sql += "SELECT TOP 1 mcps_prd_sled ";
            sql += "FROM TBL_MAP_CUST_PRD_SLED WITH (NOLOCK) ";
            sql += "WHERE mcps_entity_id = '" + xEntityID + "' ";
            sql += "AND mcps_branch_id = '" + xBranchID + "' ";
            sql += "AND mcps_cust_code1 = '" + txtKdOutlet1.Text.Trim() + "' ";
            sql += "AND mcps_cust_code2 = '" + lblKdOutlet2.Text.Trim() + "' ";
            sql += "AND mcps_prd_code = '" + pcode + "' ";
            sql += "AND mcps_prd_grade = '" + grade + "' ";
            sql += "AND mcps_prd_size = '" + size + "' ";

            DataTable dt = _clsGlobal.ExecDT(sql);

            if (dt.Rows.Count > 0)
                return dt.Rows[0]["mcps_prd_sled"].ToString().Trim();

            return "";
        }

        private void ApplyMappedSLEDToAllRows()
        {
            for (int i = 0; i < dgvSalesDetail.Rows.Count; i++)
            {
                string pcode = dgvSalesDetail.Rows[i].Cells["wsd_prd_master_code"].Value == null
                    ? ""
                    : dgvSalesDetail.Rows[i].Cells["wsd_prd_master_code"].Value.ToString().Trim();

                string grade = dgvSalesDetail.Rows[i].Cells["wsd_grade"].Value == null
                    ? ""
                    : dgvSalesDetail.Rows[i].Cells["wsd_grade"].Value.ToString().Trim();

                string size = dgvSalesDetail.Rows[i].Cells["wsd_prd_size"].Value == null
                    ? ""
                    : dgvSalesDetail.Rows[i].Cells["wsd_prd_size"].Value.ToString().Trim();
                ...
                if (IsActiveFlagSLED())
                {
                    dgvSalesDetail.Rows[i].Cells["wsd_sled"].Value = GetMappedSLED(pcode, grade, size);
                }
            }
        }
    }
