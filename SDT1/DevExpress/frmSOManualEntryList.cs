            ...
            using (SqlConnection con = new SqlConnection(clsGlobal.CONNECTION_STRING))
            {
                ...
                try
                {
                    ...
                    if (dtDD.Rows.Count > 0)
                    {
                        ...
                        for (int f = 1; f < gvDisc.Columns.Count && f < 41; f++)
                        {
                            bHiddenCol = true;
                            for (int rowHandle = 0; rowHandle < gvDisc.RowCount; rowHandle++)
                            {
                                object val = gvDisc.GetRowCellValue(rowHandle, gvDisc.Columns[f]);
                            ...
                            gvDisc.Columns[f].Visible = bHiddenCol;
                        }
                    }
                    else
                    {
