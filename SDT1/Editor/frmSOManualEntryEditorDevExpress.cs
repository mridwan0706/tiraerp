    ...
    public partial class frmSOManualEntryEditor : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void MarkMandatoryValidationErrors()
        {
            ...
            if (rbCashDiscP.Checked && !string.IsNullOrWhiteSpace(txtCashDiscP.Text))
            {
                ...
                {
                    if (cmbBebanDisc.SelectedValue != null && Convert.ToString(cmbBebanDisc.SelectedValue) == "0")
        ...
        private void frmSOManualEntryEditor_Load(object sender, EventArgs e)
        {
            ...
            BindTOP();
            cbTOP.SelectedIndexChanged -= cbTOP_SelectedIndexChanged;
            bindTipeOrder();
        ...
        private void bindTipeOrder()
        {
            ...
            {
                cbTipeOrder.DataSource = _clsGlobal.ExecDTTrans(strSQL);
            }
            cbTipeOrder.ValueMember = "code";
            cbTipeOrder.DisplayMember = "order_type";

            for (lCounter = 0; lCounter < cbTipeOrder.Items.Count; lCounter++)
            {
                g_bSOType = false;
                cbTipeOrder.SelectedIndex = lCounter;
                g_bSOType = true;
                //DataRow selectedDataRow = ((DataRowView)cbTipeOrder.SelectedItem).Row;
                string idO = cbTipeOrder.SelectedValue.ToString();
                ...
                if (lCounter == cbTipeOrder.Items.Count)
                {
                    g_bSOType = false;
                    cbTipeOrder.SelectedIndex = 0;
                    g_bSOType = true;
                }
            }
        }
        ...
        private void btnPopUpOutlet_Click(object sender, EventArgs e)
        {
            ...
            if (frm.ArrField != null)
            {
                ...
                {
                    g_bEdit = false;
                    for (lCounter = 0; lCounter < cbPembyFaktur.Items.Count; lCounter++)
                    {
                        cbPembyFaktur.SelectedIndex = lCounter;
                        string cdPF = frm.ArrField[5].Trim();
                        string cbPemFak = cbPembyFaktur.SelectedValue.ToString();
                        if (cbPemFak == cdPF)
                        {
                            g_bEdit = true;
                ...
                if (_xFlagDC == "Y")
                {
                    ...
                    if (dtFill2.Rows.Count > 0)
                    {
                        ...
                        {
                            for (lCounter = 0; lCounter < cbTOP.Items.Count; lCounter++)
                            {
                                cbTOP.SelectedIndex = lCounter;
                                string cdPF = dtFill3.Rows[0]["sgl_group_top"].ToString().Trim();
                                string vTOP = cbTOP.SelectedValue.ToString();
                                if (cbTOP.SelectedValue.ToString() == cdPF)
                                {
                                    cbTOP.Enabled = false;
                                    break;
                                }
                        ...
                        string vPF = cbPembyFaktur.SelectedValue.ToString();
                        ...
                        string vPF1 = cbPembyFaktur.SelectedValue.ToString();
                        //jika tipe pembayaran T, TOP tidak berubah, 21-03-2019
                        if (vPF1 == "T")
                        {
                            cbTOP.SelectedIndex = 0;
                        ...
                        else
                        {
                            ...
                            if (frm.ArrField[8].ToString().Trim() == "Y")
                            {
                                for (lCounter = 0; lCounter < cbTOP.Items.Count; lCounter++)
                                {
                                    ...
                                    if (cbTOP.SelectedValue.ToString() == __top)
                                    {
                                        g_bEdit = true;
                ...
                else
                {
                    ...
                    if (cbPembyFaktur.SelectedValue.ToString() == "T")
                    {
                        cbTOP.SelectedIndex = 0;
                    }
                    else if (dtFillcheckTopByDivision.Rows.Count > 0)
        ...
        private CalcDiscBackupState CreateCalcDiscBackupState()
        {
            ...
            try
            {
                ...
                state.Pembayaran = cbPembyFaktur.SelectedValue;
                state.TOP = cbTOP.SelectedValue;
                state.TipeOrder = cbTipeOrder.SelectedValue;
            }
            catch
            {
        ...
        private void RestoreOutletAfterCalcDiscIfBlank(CalcDiscBackupState state)
        {
            try
            {
                ...
                if (state.Pembayaran != null) cbPembyFaktur.SelectedValue = state.Pembayaran;
                if (state.TOP != null) cbTOP.SelectedValue = state.TOP;
                if (state.TipeOrder != null) cbTipeOrder.SelectedValue = state.TipeOrder;
            }
            catch
            {
                // Safety only.
            ...
            if (_xFlagDC == "Y")
            {
                if (cbPembyFaktur.Text == "")
                {
                    ...
                    strSQL = "SELECT gh_group_menu_id FROM GS_GEN_HARDCODED WITH (NOLOCK)  " +
                        "WHERE gh_function_name ='CUSTPAYTYPE' AND gh_function_code = '" + cbPembyFaktur.SelectedValue + "'";
                    dtFill1 = _clsGlobal.ExecDT(strSQL);
        ...
        {
            if (cbPembyFaktur.SelectedValue == "T")
            {
                cbTOP.SelectedValue = "0";
            }
            else if (cbPembyFaktur.SelectedValue == "K")
            {
                if (txtSalesID.Text == "")
                {
                ...
                if (dtFill1.Rows.Count > 0)
                {
                    if (dtFill1.Rows[0]["topnya"].ToString().Trim() != "")
                    {
                        ...
                        {
                            cbTOP.SelectedIndex = lCounter;
                            if (cbTOP.SelectedValue == dtFill1.Rows[0]["topnya"].ToString().Trim())
                            {
                                cbTOP.Enabled = false;
                            }
                        }
                    ...
                    else
                    {
                        ...
                        if (dtFill2.Rows.Count > 0)
                        {
                            ...
                            {
                                cbTOP.SelectedIndex = lCounter;
                                if (cbTOP.SelectedValue == dtFill2.Rows[0]["topnya"].ToString().Trim())
                                {
                                    cbTOP.Enabled = false;
                                }
                            }
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //if (dtFill2.Rows.Count > 0)
                //{
                //    cbPembyFaktur.DataSource = _clsGlobal.ExecDT(strSQL);
                //    cbPembyFaktur.ValueMember = "code";
                //    cbPembyFaktur.DisplayMember = "name";
                //}
        ...
        private void dgvSalesDetail_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (dgvSalesDetail.Rows[row].Cells["wsd_prd_master_code"].Selected == true)
                {
                    ...
                    //DataRow selectedDataRow = ((DataRowView)cbTipeOrder.SelectedItem).Row;
                    int idO = Convert.ToInt32(cbTipeOrder.SelectedValue.ToString());
                    string NameO = cbTipeOrder.Text;
                    ...
                    if (g_bTOPFlag == true)
                    {
                        DataRow selectedDataRow1 = ((DataRowView)cbTOP.SelectedItem).Row;
        ...
        private void TopTOPDivisionRenew()
        {
            ...
            if (recal)
            {
                if (dgvSalesDetail.Rows.Count > 0)
                {
                    ...
                    {
                        if (cbTOP.SelectedValue.ToString().Trim() != dtCheck2.Rows[0].ToString().Trim())
                        {
                            for (lCounter = 0; lCounter < cbTOP.Items.Count; lCounter++)
                            {
                                cbTOP.SelectedIndex = lCounter;
        ...
        private void cmbReasonGrid_OnTextChange(object sender, EventArgs e)
        {
            ...
            string selectedReason = Convert.ToString(cmbReasonGrid.SelectedValue);
            ...
            else if (e.KeyCode == Keys.F4)
            {
                ...
                if (dgvSalesDetail.Rows[row].Cells["wsd_prd_master_code"].Selected == true)
                {
                    ...
                    if (g_bTOPFlag == true)
                    {
                        ...
                        strSQL = strSQL + "  AND cm_entity = '" + xEntityID + "'";
                        strSQL = strSQL + "  AND cm_branch = '" + xBranchID + "'";
                        strSQL = strSQL + "  AND pptc_term_code >= '" + cbTOP.SelectedValue.ToString().Trim() + "'";
                        strSQL = strSQL + " ) TOPPRD ON prm_prd_line_code = map_prdline_code ";
                    }
                    if (sTypeOrder == "C" && sTypeSales == "C")
                    {
                    ...
                    //if (isMultisource)
                    //{
                    //    if (cbFlagSloc.SelectedValue.ToString().ToUpper().Equals("COC"))
                    //    {
                    //        strSQL = strSQL + " AND cocline is not null";
                    //    }
                    //    else
                    ...
                    if (frm.ArrField != null)
                    {
                        if (isNefoKAM)
                        {
                            ...
                            if (isTOPDivision == "Y")
                            {
                                ...
                                if (cbPembyFaktur.SelectedValue.ToString() == "T" && dtCheck.Rows.Count > 0)
                                {
                                    sValidDivProd = true;
                                    if (dgvSalesDetail.Rows.Count > 1)
                                    {
        ...
        private bool checkKAMTOPKAM(int mode, string sPCode, string sGrade, string sSize, string sCustCode1, string sCustCode2, string sSpgmCode)
        {
            ...
            else
            {
                ...
                if (dtNewProduct.Rows.Count > 0 && dtExistProduct.Rows.Count > 0)
                {
                    ...
                    int existsnoadays = Convert.ToInt32(cbTOP.SelectedValue.ToString().Replace("P", "").ToString().Trim());
                ...
                {
                    TOPDivision = cbTOP.SelectedValue.ToString().Trim();
                }
        ...
        private void dgvSalesDetail_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (IsGridColumn(dgvSalesDetail, e, COL_PCODE))
            {
                ...
                if (dgvSalesDetail.Rows[row].Cells["wsd_prd_master_code"].Value.ToString() != "")
                {
                    ...
                    //DataRow selectedDataRow = ((DataRowView)cbTipeOrder.SelectedItem).Row;
                    string idO = Convert.ToString(cbTipeOrder.SelectedValue.ToString());
                    ...
                    if (dtFill3.Rows.Count > 0)
                    {
                        ...
                        if (isNefoKAM)
                        {
                            ...
                            if (isTOPDivision == "Y")
                            {
                                ...
                                if (cbPembyFaktur.SelectedValue == "T" && dtCheck.Rows.Count > 0)
                                {
                                    sValidDivProd = true;
                                    if (dgvSalesDetail.Rows.Count > 1)
                                    {
                    ...
                    else
                    {
                        ...
                        if (!_clsGlobal.IsNefoForBL)
                        {
                            ...
                            for (lCounter = 0; lCounter < cbPembyFaktur.Items.Count; lCounter++)
                            {
                                ...
                                string vTOP = cbPembyFaktur.SelectedValue.ToString();
                                if (vTOP == cdPF)
                                {
                                    g_bEdit = true;
                                    break;
...
ORDER BY reason";
        ...
        private void SetGridSODetail(DataGridView dgv)
        {
            ...
            {
                dgv.Columns["reason"].Visible = false;
                dgv.Columns["wsd_req_order_xqty"].Visible = false;
                dgv.Columns["wsd_sled"].Visible = false;
            }

            dgv.Columns["wsd_sap_unit_price"].Visible = false;
            ...
            EnsureReasonSearchLookUpEditor();
            dgv.Columns["wsd_sap_unit_price"].HeaderText = "SAP Unit Price";
            ...
            dgv.Columns["wsd_tot_disc_pc"].DefaultCellStyle.Format = "#,##0.00";
        ...
        private void BindTOP()
        {
            try
            {
                ...
                if (g_bTOPFlag == true)
                {
                    ...
                    {
                        //cbTOP.Items.Clear();
                        cbTOP.DataSource = _clsGlobal.ExecDT(strSQL);
                        cbTOP.ValueMember = "code";
                        cbTOP.DisplayMember = "pptc_term_des";
                    }
                ...
                else
                {
                    ...
                    if (dtFill2.Rows.Count > 0)
                    {
                        //cbTOP.DataSource = null;
                        //cbTOP.Items.Clear();
                        cbTOP.DataSource = _clsGlobal.ExecDT(strSQL);
                        cbTOP.ValueMember = "code";
                        cbTOP.DisplayMember = "pptc_term_desc";
                    }
                ...
                if (dtFill.Rows.Count > 0)
                {
                    ...
                    if (dtFill2.Rows.Count > 0)
                    {
                        ...
                        if (dtFill3.Rows.Count > 0)
                        {
                            ...
                            for (int i = 0; i < cbTOP.Items.Count; i++)
                            {
                                cbTOP.SelectedIndex = i;
                                //DataRow selectedDataRow = ((DataRowView)cbTOP.SelectedItem).Row;
                                string idO = cbTOP.SelectedValue.ToString();
        ...
        private void BindBebanDisc()
        {
            ...
            cmbBebanDisc.ValueMember = "code";
            cmbBebanDisc.DisplayMember = "Descr";
            cmbBebanDisc.DataSource = dt;
        }
        ...
        private void BindPembyFaktur()
        {
            ...
            if (dtFill2.Rows.Count > 0)
            {
                cbPembyFaktur.DataSource = _clsGlobal.ExecDT(strSQL);
                cbPembyFaktur.ValueMember = "code";
                cbPembyFaktur.DisplayMember = "gh_function_code";
            }
        }
        ...
        private void pFillData(string sNoOrder, string sEntitys, string sBranchs, bool bCalcDisc)
        {
            try
            {
                ...
                if (dtFill1.Rows.Count > 0)
                {
                    ...
                    if (isMultisource)
                    {
                        cbSource.SelectedValue = dtFill1.Rows[0]["wsh_qasir_flag"].ToString();
                        cbFlagSloc.SelectedValue = dtFill1.Rows[0]["wsh_flag_sloc"].ToString();
                    }
                    cbfullfilment.Checked = false;
                    ...
                    cmbBebanDisc.SelectedValue = dtFill1.Rows[0]["wsh_beban_cashdisc"].ToString();
                    ...
                    else
                    {
                        ...
                        for (lCounter = 0; lCounter < cbTipeOrder.Items.Count; lCounter++)
                        {
                            ...
                            cbTipeOrder.SelectedIndex = lCounter;
                            g_bSOType = true;
                            string vTOP = cbTipeOrder.SelectedValue.ToString();
        ...
        private bool fChangeSOType()
        {
            ...
            {
                //DataRow selectedDataRow = ((DataRowView)cbTipeOrder.SelectedItem).Row;
                int idO = Convert.ToInt32(cbTipeOrder.SelectedValue);
                string NameO = cbTipeOrder.Text;
            ...
            try
            {
                ...
                if (rbCashDiscP.Checked)
                {
                    if (!string.IsNullOrEmpty(txtCashDiscP.Text.Trim()))
                    {
                        ...
                        {
                            if (cmbBebanDisc.SelectedValue.ToString() == "0")
                            {
                ...
                if (isMultisource)
                {
                    ...
                    strSQL += " and pms_flag_source = '" + cbSource.SelectedValue.ToString() + "' ";
                    strSQL += " and pms_flag_sloc = '" + cbFlagSloc.SelectedValue.ToString() + "' ";
        ...
        private bool fSaveSO(string sCredLimit, bool bCalcDisc)
        {
            ...
            try
            {
                ...
                //DataRow selectedDataRow = ((DataRowView)cbPembyFaktur.SelectedItem).Row;
                string idO = cbPembyFaktur.SelectedValue.ToString();
                ...
                if (isMultisource)
                {
                    strSQL = strSQL + ",'" + cbSource.SelectedValue.ToString() + "' "; //'qasirflag
                }
                else
                {
                    if (QasirFlag != "")
                ...
                if (rbCashDiscP.Checked)
                {
                    if (!string.IsNullOrEmpty(txtCashDiscP.Text.Trim().ToString()))
                    {
                        ...
                        {
                            _beban = cmbBebanDisc.SelectedValue.ToString();
                        }
                    }
                ...
                if (isMultisource)
                {
                    strSQL = strSQL + ",'" + cbFlagSloc.SelectedValue.ToString() + "' "; // wsh_flag_sloc ";
                }
                ...
                for (int i = 0; i < dgvSalesDetail.Rows.Count; i++)
                {
                    if (dgvSalesDetail.Rows[i].Cells["wsd_prd_master_code"].Value.ToString() != "")
                    {
                        ...
                        strSQL = strSQL + " ,wsd_tax_pct,wsd_tax_amount,wsd_gross_net,wsd_cash_disc,wsd_line_type,wsd_price_date,wsd_price_code,wsd_reason,wsd_sap_unit_price,wsd_beban_cashdisc,wsd_sled,wsd_req_order_qty,wsd_req_order_xqty,wsd_req_order_amt)";
            ...
            try
            {
                ...
                for (lCounter = 0; lCounter < dgvSalesDetail.Rows.Count; lCounter++)
                {
                    if (dgvSalesDetail.Rows[lCounter].Cells["wsd_prd_master_code"].Value.ToString() != "")
                    {
                        ...
                        strSQL = strSQL + " ,wsd_invoice_process_by,wsd_entry_date,wsd_user_id,wsd_curr_code,wsd_rate,wsd_org_het_unit_price,wsd_org_total_so_amount,wsd_uom_code";
                        strSQL = strSQL + " ,wsd_uom_convert_mid,wsd_uom_convert_big,wsd_uom_org_qty,wsd_tot_disc_pc,wsd_tot_disc_1,wsd_tot_disc_2,wsd_tot_tpr_amt,wsd_tax_code";
                        strSQL = strSQL + " ,wsd_tax_pct,wsd_tax_amount,wsd_gross_net,wsd_cash_disc,wsd_line_type,wsd_price_date,wsd_price_code,wsd_reason,wsd_sap_unit_price,wsd_sled,wsd_req_order_qty,wsd_req_order_xqty,wsd_req_order_amt)";
                        strSQL = strSQL + " values ( '";
                        strSQL = strSQL + _xEntityID + "' , '"; //'wsd_entity_id
                        strSQL = strSQL + lblOrderType.Text + "' , '"; //'wsd_so_type
                        strSQL = strSQL + sSONo + "', '";  //'wsd_so_seq_no
        ...
        private void cbFlagSloc_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                if (cbFlagSloc.SelectedValue != null && cbFlagSloc.SelectedValue.ToString() == "DDE")
                {
        ...
        private void getReason()
        {
            ...
            cmbReasonGrid.DataSource = _clsGlobal.ExecDT(strSQL);
            cmbReasonGrid.ValueMember = "reason";
            cmbReasonGrid.DisplayMember = "reason";
        }
...
namespace TIRASnDNet.PROCESS.SO.SOManualEntry
{
    ...
    public class DataGridViewColumn
    {
        ...
        public string HeaderText
        {
            ...
            set { _headerText = value; ApplyToGridColumn(); }
        }
    ...
    public class DataGridView : GridControl
    {
        ...
        public DataGridView()
        {
            ...
            _view.HorzScrollVisibility = DevExpress.XtraGrid.Views.Base.ScrollVisibility.Always;
            _view.VertScrollVisibility = DevExpress.XtraGrid.Views.Base.ScrollVisibility.Always;
            _view.OptionsBehavior.Editable = true;
            _view.OptionsSelection.MultiSelect = true;
            _view.FocusedRowChanged += (s, e) => SelectionChanged?.Invoke(this, EventArgs.Empty);
            _view.CellValueChanged += (s, e) =>
            {
                AutoBestFitColumn(e.Column);
                CellValueChanged?.Invoke(this, new DataGridViewCellEventArgs(GetColumnIndex(e.Column), e.RowHandle));
            };
            _view.RowCellClick += (s, e) => { var args = new DataGridViewCellEventArgs(GetColumnIndex(e.Column), e.RowHandle); CellClick?.Invoke(this, args); CellContentClick?.Invoke(this, args); };
            _view.HiddenEditor += (s, e) => { var col = _view.FocusedColumn; CellEndEdit?.Invoke(this, new DataGridViewCellEventArgs(GetColumnIndex(col), _view.FocusedRowHandle)); };
            _view.ShownEditor += (s, e) => EditingControlShowing?.Invoke(this, new DataGridViewEditingControlShowingEventArgs(_view.ActiveEditor));
        }
        internal GridView FocusedView { get { return _view; } }
        ...
        public bool IsCurrentCellDirty { get { return _view.IsEditing; } }
        ...
        {
            get { return _view.FocusedColumn == null ? null : new DataGridViewCell(this, _view.FocusedRowHandle, _view.FocusedColumn.FieldName); }
            set { if (value != null) SetCurrentCell(value.RowIndex, value.OwningColumnName); }
        }
        public DataGridViewRow CurrentRow { get { return _view.FocusedRowHandle < 0 ? null : new DataGridViewRow(this, _view.FocusedRowHandle); } }
        internal void SetCurrentCell(int rowIndex, string fieldName)
        {
            _view.FocusedRowHandle = rowIndex;
            var col = _view.Columns.ColumnByFieldName(fieldName) ?? _view.Columns[fieldName];
            if (col != null) _view.FocusedColumn = col;
        }
        internal object GetCellValue(int rowIndex, string fieldName) { return rowIndex < 0 ? null : _view.GetRowCellValue(rowIndex, fieldName); }
        internal void SetCellValue(int rowIndex, string fieldName, object value) { if (rowIndex >= 0) _view.SetRowCellValue(rowIndex, fieldName, value); }
        internal bool GetCellReadOnly(string fieldName)
        {
            GridColumn col = _view.Columns.ColumnByFieldName(fieldName) ?? _view.Columns[fieldName];
        ...
        public void EndEdit() { _view.CloseEditor(); _view.UpdateCurrentRow(); }
        public void ClearSelection() { _view.ClearSelection(); }
        public void DeleteRow(int rowIndex) { if (rowIndex >= 0) _view.DeleteRow(rowIndex); }
        ...
        private GridColumn GetGridColumnByCompatIndex(int columnIndex)
        {
            ...
            if (table != null && columnIndex < table.Columns.Count)
            {
                ...
                return _view.Columns.ColumnByFieldName(fieldName) ?? _view.Columns[fieldName];
            }

            return columnIndex < _view.Columns.Count ? _view.Columns[columnIndex] : null;
        }
        ...
        public Rectangle GetCellDisplayRectangle(int columnIndex, int rowIndex, bool cutOverflow)
        {
            try
            {
                ...
                _view.LayoutChanged();
                DevExpress.XtraGrid.Views.Grid.ViewInfo.GridViewInfo viewInfo = _view.GetViewInfo() as DevExpress.XtraGrid.Views.Grid.ViewInfo.GridViewInfo;
    ...
    // ============================================================
    // DevExpress native editor compatibility layer (single-file)
    // ============================================================
    // Mengganti TextBox/ComboBox/CheckBox WinForms menjadi editor DevExpress
    // tanpa mengubah business logic lama yang masih memakai Text, DataSource,
    // SelectedValue, SelectedIndex, Items.Count, Checked, ReadOnly, dll.
    public class DevExpressTextEditCompat : DevExpress.XtraEditors.TextEdit
    {
        public DevExpressTextEditCompat()
        {
        ...
        {
            get { return this.Properties.CharacterCasing; }
            set { this.Properties.CharacterCasing = value; }
        }

        public System.Windows.Forms.BorderStyle BorderStyle
        {
            get { return System.Windows.Forms.BorderStyle.FixedSingle; }
            set { }
        }
        ...
        public System.Windows.Forms.HorizontalAlignment TextAlign
        {
            get
            {
                ...
                if (this.Properties.Appearance.TextOptions.HAlignment == DevExpress.Utils.HorzAlignment.Center)
            ...
            set
            {
                ...
                else if (value == System.Windows.Forms.HorizontalAlignment.Center)
    ...
    public class DevExpressCheckEditCompat : DevExpress.XtraEditors.CheckEdit
    {
        public bool UseVisualStyleBackColor { get; set; }

        public DevExpressCheckEditCompat()
        {
            this.Properties.AutoWidth = true;
        }
    }

    public class DevExpressLookupComboCompat : DevExpress.XtraEditors.LookUpEdit
    {
        ...
        public DevExpressLookupComboCompat()
        {
        ...
        public System.Windows.Forms.ComboBoxStyle DropDownStyle
        {
            get
            {
                return this.Properties.TextEditStyle == DevExpress.XtraEditors.Controls.TextEditStyles.DisableTextEditor
                    ? System.Windows.Forms.ComboBoxStyle.DropDownList
                    : System.Windows.Forms.ComboBoxStyle.DropDown;
            }
            set
            {
                this.Properties.TextEditStyle = value == System.Windows.Forms.ComboBoxStyle.DropDownList
        ...
        public object DataSource
        {
            ...
            set
            {
                ...
                RefreshColumns();
                _selectedIndex = (_dataSource != null && _dataSource.Rows.Count > 0) ? 0 : -1;
                if (_selectedIndex >= 0) SetEditValueFromIndex(_selectedIndex);
            }
        }
        ...
        public string ValueMember
        {
            ...
            set { _valueMember = value; this.Properties.ValueMember = value; RefreshColumns(); }
        }

        public string DisplayMember
        {
            get { return _displayMember; }
            set { _displayMember = value; this.Properties.DisplayMember = value; RefreshColumns(); }
        }
        ...
        public object SelectedValue
        {
            ...
            set
            {
                ...
                _selectedIndex = FindIndexByValue(value);
            }
        }
        ...
        public object SelectedItem
        {
            get
            {
                try
                {
                    if (_dataSource == null || _selectedIndex < 0 || _selectedIndex >= _dataSource.DefaultView.Count)
                        return null;
                    return _dataSource.DefaultView[_selectedIndex];
                }
                catch { return null; }
            }
            set
            {
                try
                {
                    ...
                    if (drv == null || _dataSource == null)
                    {
                        this.SelectedIndex = -1;
                        return;
                    }

                    for (int i = 0; i < _dataSource.DefaultView.Count; i++)
                    {
                        if (object.ReferenceEquals(_dataSource.DefaultView[i].Row, drv.Row))
                        {
        ...
        public int SelectedIndex
        {
            ...
            set
            {
                ...
                SetEditValueFromIndex(value);
                RaiseSelectedIndexChanged();
            }
        }

        public CompatComboItems Items
        {
            get { return new CompatComboItems(_dataSource); }
        }

        protected override void OnEditValueChanged()
        {
            base.OnEditValueChanged();
            _selectedIndex = FindIndexByValue(this.EditValue);
            RaiseSelectedIndexChanged();
        }
        ...
        private void SetEditValueFromIndex(int index)
        {
            if (_dataSource == null || index < 0 || index >= _dataSource.Rows.Count)
            {
                this.EditValue = null;
                return;
            }

            string valueCol = !string.IsNullOrEmpty(_valueMember) ? _valueMember : _dataSource.Columns[0].ColumnName;
            this.EditValue = _dataSource.Rows[index][valueCol];
        }

        private int FindIndexByValue(object value)
        {
            if (_dataSource == null || string.IsNullOrEmpty(_valueMember)) return -1;
            string val = Convert.ToString(value);
            for (int i = 0; i < _dataSource.Rows.Count; i++)
            {
                if (Convert.ToString(_dataSource.Rows[i][_valueMember]) == val) return i;
            }
            return -1;
        }
        ...
        private void RefreshColumns()
        {
            try
            {
                ...
                string displayCol = !string.IsNullOrEmpty(_displayMember) ? _displayMember : _dataSource.Columns[0].ColumnName;
                this.Properties.Columns.Add(new DevExpress.XtraEditors.Controls.LookUpColumnInfo(displayCol, displayCol));
            }
            catch { }
        }

        public class CompatComboItems
        {
            private readonly DataTable _table;
            public CompatComboItems(DataTable table) { _table = table; }
            public int Count { get { return _table == null ? 0 : _table.Rows.Count; } }

            public void Clear()
            {
                if (_table != null) _table.Rows.Clear();
            }

            public int Add(object item)
            {
                if (_table == null) return -1;
                DataRow row = _table.NewRow();
                if (_table.Columns.Count > 0) row[0] = item;
                _table.Rows.Add(row);
